...
The other link of interest is the one between the mosquitto broker and the cloud service provider, the main aspect here is that in the middle there is the transparent bridge that acts on the messages, but even if the message size increases and the protocol used to exchange messages is TCP (that uses in general more messages than UDP), the operation done by the transparent bridge are done on the edge and for this reason the presence of a more reliable bandwidth is almost guaranteed.<br><br>
***Latency***: instead regarding the latency of the system we have that usually is under 2s between when the sensor register a new value and when this new value appears on the web application, the same is true for the other direction: there is a latency under 2s between when a button is clicked on the web app and when the system acts accordingly. <br>This latency in a case like this is not a problem at all, indeed the system does not have strcit requirements neither on how fast to act when a command is sent by the user nor on how fast the data must be shown to the user. Mainly this is due to what the system is though for, indeed under noraml circumstances is rare for values like temperature and humidity to drastically change and even if we are in those rare circumstances, excluding the user response time, 4s is small enough to prevent "damages".
