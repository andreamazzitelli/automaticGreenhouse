...
static dht_t dev;
static int16_t goal_temp = 200; //need to better handle this
...
static void on_pub(const emcute_topic_t *topic, void *data, size_t len){
    ...
    if(*in == 'g'){ 
       ...
       //change_relay_state(""); //creates issues
   }
   return;
}
...
static int address_setup(char *name, char *ip_address){
    ...
    ipv6_addr_t ip_addr;
    uint16_t flag = GNRC_NETIF_IPV6_ADDRS_FLAGS_STATE_VALID | (IPV6_PREFIX_LEN << 8U); //setting the flag fot the set function
...
static void change_relay_state(char *status){
    ...
    if(relay_state && strcmp(status, RELAY_OFF)){
        //puts("relay off");
        gpio_clear(pin_out);
        relay_state = false;
        return;
    }
    else if(!relay_state && strcmp(status, RELAY_ON)){
        //puts("relay on");
        gpio_set(pin_out);
        relay_state = true;
        return;
...
void *temp_relay_logic(void *arg){
    ...
    while(1){
        ...
        if(temp > goal_temp)change_relay_state(RELAY_OFF);
        else if(temp <= goal_temp)change_relay_state(RELAY_ON);
